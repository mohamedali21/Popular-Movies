...
public class FetchTrailersTask extends AsyncTask<String,Void,String> {
    ...
    Context context;
    public FetchTrailersTask(Context con){
        context=con;
    }
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            // Possible parameters are avaiable at OWM's moivie API page, at
            // http://api.themoviedb.org/API#movie
            final String MOVIE_BASE_URL =
                    "http://api.themoviedb.org/3/movie/";
            final String s="/trailers";
        ...
        if (mstring != null)
        {
            try {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    String key = MovieObject.getString("source");
                    ...
                    if (sql.isOpen()) {
                        if (!(movieHelper.checkTralers(key)))
                            Log.d("VALUE", movieHelper.insertTrailers(movie) + "");
                    } else
                        Log.d("DB", "Closed");
                }
